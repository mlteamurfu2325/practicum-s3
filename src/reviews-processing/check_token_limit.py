...
def check_and_truncate_tokens(parquet_path, token_limit, updated_parquet_path):
    ...
    def process_text(text):
        if pd.isna(text):
            return pd.Series(
                {"tokens_num": 0, "text": text, "is_trunc_for_token_limit": 0}
            )
        ...
        if token_count > token_limit:
            ...
            return pd.Series(
                {"tokens_num": token_count, "text": text, "is_trunc_for_token_limit": 0}
            )
    ...
    print(
        f"Updated Parquet file with 'tokens_num' and 'is_trunc_for_token_limit' columns saved to '{updated_parquet_path}'."
    )
    ...
    if not still_exceeds.empty:
        ...
        still_exceeds.to_csv(
            "reviews_exceeding_token_limit_after_truncation.csv", index=False
        )
        print(
            "Details have been saved to 'reviews_exceeding_token_limit_after_truncation.csv'."
        )
    else:
