...
class ModelNameFilter(logging.Filter):
    """Filter that adds model name to log records."""
    
    def __init__(self, model_name):
        super().__init__()
        self.model_name = model_name
        self.display_name = AVAILABLE_MODELS.get(model_name, model_name)

    def filter(self, record):
        record.model_name = f"[{self.display_name}]"
        return True
...
class SessionStateHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        # Format message with model name
        log_entry = f"{timestamp} [{record.levelname}] {record.model_name} {record.msg}"
...
class ReviewGenerator:
    ...
    def __init__(self, model_name=None):
        ...
        # Add model name filter to logger
        self.model_filter = ModelNameFilter(self.model_name)
        self.logger.addFilter(self.model_filter)
