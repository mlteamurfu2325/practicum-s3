...
Задача: Проверить пригодность пользовательского ввода для генерации отзыва.

Входные данные: "{input_text}"

Проверь следующие критерии и ответь в формате JSON:
{{
    "is_valid": true/false,
...
3. Релевантность для отзыва о месте/заведении
4. Осмысленность текста
...
Оцени по критериям и верни результат в формате JSON:
{{
    "scores": {{
